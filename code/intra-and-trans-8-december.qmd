---
title: "Combine the Trans-Atlantic and Itra-American datasets"
format: html
editor: source
execute:
  enabled: true
...
2. Voyages-IAm-May2023.csv, which is the intra-American data from
...
The codebook changed from the 2022 version to the 20203. In the earlier version, region codes in the 8000's were "Other," now they are "Asia," and codes in the 9000's are "Other." However, the downloadable dataset are still in the 2022 coding. 
...
path <- "/Users/richardevans/Library/Mobile Documents/com~apple~CloudDocs/2-my-work/combined-slave-voyages/data/files-for-import/"
...
So now there are 315 varibles, one of which is the new __intra_or_trans__ varaible.


Now check which variables are are populated for one voyage by empty for the other 

```{r}
#| lablel: "vars missing data"
#| include: TRUE
#| results: "asis"
#| message: FALSE
#| warning: FALSE
...
# Identify variables that are all missing for each group
missing_vars <- function(data, group_var, target_group) {
  # Filter rows by the group
  group_data <- data[data[[group_var]] == target_group, ]
  
  # Check which columns are entirely NA
  sapply(group_data, function(col) all(is.na(col)))
}

# Variables missing entirely for intra group
missing_intra <- missing_vars(df, "intra_or_trans", "intra_american")

# Variables missing entirely for trans group
missing_trans <- missing_vars(df, "intra_or_trans", "trans_atlantic")

# Combine results
all_missing_vars <- list(
  intra_missing = names(missing_intra[missing_intra]),
  trans_missing = names(missing_trans[missing_trans])
)
...
# setdiff(c(1,2,3),c(3,4,5)) is c(1,2)
# so setdiff(missing_intra, missing_trans)
# is vars with all missing data in intra but not trans


# Find variables unique to each set and common to both sets
unique_and_common_vars <- function(missing_intra, missing_trans) {
  # Variables common to both sets
  common_vars <- intersect(missing_intra, missing_trans)
  
  # Variables unique to each set
  unique_to_intra <- setdiff(missing_intra, missing_trans)
  unique_to_trans <- setdiff(missing_trans, missing_intra)
  
  # Return results as a list
  list(
    common_vars_missing_all_data = common_vars,
    intra_vars_missing_all_data = unique_to_intra,
    trans_vars_missing_all_data = unique_to_trans
  )
}

# Apply to the previously identified sets
results <- unique_and_common_vars(
  names(missing_intra[missing_intra]),
  names(missing_trans[missing_trans])
)
...
results_df <- data.frame(
  Variable_missing_all_data = c(results$common_vars_missing_all_data, results$intra_vars_missing_all_data, results$trans_vars_missing_all_data),
  Voyage_missing_data = c(
    rep("Both", length(results$common_vars_missing_all_data)),
    rep("Intra-American", length(results$intra_vars_missing_all_data)),
    rep("Trans-Atlantic", length(results$trans_vars_missing_all_data))
  )
)

# Display the table
results_table <- results_df %>%
  arrange(Voyage_missing_data)

print(results_table)

results_df|> kable(caption = "Variables with no data")
```
...
table(df1$PLACCONS_names)
```

Now there are `r ncol(df1) - ncol(df)` new variables
...
### This finds where numeric code replacement failed.
# I other words this is a check that _names variables got names and don't have all codes 



# Custom sorting function
# puts the numbers near the top
sort_custom <- function(x) {
  x[order(as.numeric(suppressWarnings(x)), x, na.last = TRUE)]
}

# Apply custom sorting to columns containing "_names"
df_sorted <- df1 |> select(contains("_names")) |>
  mutate(across(contains("_names"), ~ sort_custom(.)))
...
# Custom sorting function that extracts only numeric values
extract_and_sort_numbers <- function(x) {
  # Convert to numeric (non-numeric values become NA) and remove NAs
...
# Apply the function to columns containing "_names"
df_sorted <- df1 |> select(contains("_names")) |>
  mutate(across(contains("_names"), ~ paste(extract_and_sort_numbers(.), collapse = ", ")))
...
df1 |> select(contains("_names")) |> glimpse()
...
head(df|> select(VOYAGEID,PLACCONS ))
...
```{r}
possible_vars <- c(
  "SPSS", "CODEBOOK", "INTRODUCTION", "CONTENTS", "TECHNICAL", "VARIABLES", 
  "DATA", "IMPUTED", "SLAXIMP", "SLAMIMP", "FORMAT", "VALUE", "LABEL", 
  "FLAG", "VOYAGEID", "VOYAGEID2", "INTRAAMER", "EVGREEN", "XMIMPFLAG", 
  "NATIONAL", "TONNAGE", "TONTYPE", "REGISREG", "OWNERA", "OWNERB", "OWNERC", 
  "OWNERD", "OWNERE", "OWNERF", "OWNERG", "OWNERH", "OWNERI", "OWNERJ", 
  "OWNERK", "OWNERL", "OWNERM", "OWNERN", "OWNERO", "OWNERP", "FATE", 
  "PORTDEP", "EMBPORT", "EMBREG", "ARRPORT", "REGARR", "RETRNREG", 
  "DEPTREGIMP", "MAJBUYPT", "MAJSELPT", "DATEDEPA", "DATEDEPB", "DATEDEPC", 
  "DATARR32", "DLSLATRA", "DLSLATRB", "DLSLATRC", "DATARR34", "DATEDEP", 
  "SLINTEND", "SLAARRIV", "SLADAFRI", "SLADVOY", "SLADAMER", "ADLT1IMP", 
  "CHIL1IMP", "MALE1IMP", "FEML1IMP", "SLAVMAX1", "SLAVEMA1", "SLAVEMX1", 
  "MENRAT1", "WOMRAT1", "BOYRAT1", "GIRLRAT1", "CHILRAT1", "MALRAT1", 
  "JAMCASPR", "SOURCEA", "SOURCEB", "SOURCEC", "SOURCED", "SOURCEE", 
  "SOURCEF", "SOURCEG", "SOURCEH", "SOURCEL", "SOURCEM", "SOURCEN", 
  "SOURCEO", "SOURCEP", "MEN", "BOY","RIG"
)
